...
public class DLList<T extends Comparable<T>> {
    ...
    private void insertionSortHelper(T item) {
        // TODO: YOUR CODE HERE
        if (this.size > 1){
            DLList<T> smallerOrEqualHalf = new DLList<>();
            DLList<T> greaterHalf = new DLList<>();

            for (Node curr = this.sentinel.next; curr != sentinel; curr = curr.next){
                if (curr.item.compareTo(item) <= 0){
                    smallerOrEqualHalf.addLast(curr.item);
                }else{
                    greaterHalf.addLast(curr.item);
                }
            }

            DLList<T> sortedList = new DLList<>();
            sortedList.append(smallerOrEqualHalf);
            sortedList.addLast(item);
            sortedList.append(greaterHalf);

            this.sentinel = sortedList.sentinel;
        }else if (size == 1){
    ...
    public DLList<T> mergeSort() {
        ...
        DLList<T> newOneHalf = oneHalf.mergeSort();
        DLList<T> newAnotherHalf = otherHalf.mergeSort();
        return newOneHalf.merge(newAnotherHalf);
    }
    ...
    public DLList<T> quicksort() {
        ...
        DLList<T> newSmallElements = smallElements.quicksort();
        DLList<T> newLargeElements = largeElements.quicksort();

        DLList<T> toReturn = new DLList<>();
        toReturn.append(newSmallElements);
        toReturn.append(equalElements);
        toReturn.append(newLargeElements);
        return toReturn;
    }
