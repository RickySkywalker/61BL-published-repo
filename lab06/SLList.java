...
public class SLList {
    ...
    public void add(int index, int x) {
        ...
        if (index >= size){
            ...
            IntListNode l1 = sentinel;
            IntListNode l2 = l1;
            for (int i = 0; i <= index; i++){
                l2 = l1;
                l1 = l1.next;
            }
            l2.next = new IntListNode(x, l1);
        }
        size++;
    ...
    private IntListNode reverseHelper(IntListNode first){
        if (first == sentinel){
            return sentinel;
        }else if (first.next == sentinel){
            return first;
        }else{
            IntListNode temp = reverseHelper(first.next);
            IntListNode last = first.next;
            last.next = first;
            first.next = sentinel;
            return temp;
        }
