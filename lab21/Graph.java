...
public class Graph {
    ...
    public Graph prims(int start) {
        ...
        TreeSet<Integer> markedVertices = new TreeSet<>();
        markedVertices.add(start);
        TreeSet<Integer> unmarkedVertices = new TreeSet<>(this.getAllVertices());
        unmarkedVertices.remove(start);

        TreeSet<Edge> setOfEdges = this.allEdges;


        while (!unmarkedVertices.isEmpty()){

            PriorityQueue<Edge> helperPQ = new PriorityQueue<>();

            for (Edge curr : setOfEdges){
                if (markedVertices.contains(curr.getSource())){
                    if (unmarkedVertices.contains(curr.getDest())){
                        helperPQ.add(curr);
                    }
                }else if (markedVertices.contains(curr.getDest())){
                    if (unmarkedVertices.contains(curr.getSource())){
                        helperPQ.add(curr);
                    }
                }
            }

            Edge edgeToAdd = helperPQ.poll();
            if (edgeToAdd != null) {
                returnGraph.addEdge(edgeToAdd);
                if (!markedVertices.contains(edgeToAdd.getDest())) {
                    markedVertices.add(edgeToAdd.getDest());
                    unmarkedVertices.remove(edgeToAdd.getDest());
                }else{
                    markedVertices.add(edgeToAdd.getSource());
                    unmarkedVertices.remove(edgeToAdd.getSource());
                }
            }else if (!unmarkedVertices.isEmpty()){
                int curr = unmarkedVertices.first();
                returnGraph.edges.put(curr, new HashSet<>());
                returnGraph.neighbors.put(curr, new HashSet<>());
    ...
    public Graph kruskals() {
        ...
        TreeSet<Integer> unmarked = new TreeSet<>(this.getAllVertices());

        int max = 0;
        for (int curr : unmarked){
            if (curr > max){
                max = curr;
            }
        }

        UnionFind connectedList = new UnionFind(max + 10);

        PriorityQueue<Edge> edgesPQ = new PriorityQueue<>(this.getAllEdges());

        while(returnGraph.allEdges.size() < this.getAllVertices().size()-1){
            Edge curr = edgesPQ.poll();
            if (curr != null) {
                if (!connectedList.connected(curr.getSource(), curr.getDest())) {
                    returnGraph.addEdge(curr);
                    connectedList.union(curr.getSource(), curr.getDest());
                }
            }else{
                break;
            }
