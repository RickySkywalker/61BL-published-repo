...
public class MemoryGame {
    ...
    public String generateRandomString(int n) {
        ...
        String initialString = Character.toString(CHARACTERS[rand.nextInt(24)]);
        String returnString = initialString;
        for (int i = 0; i < n-1; i++){
            int thisIndex = rand.nextInt(24);
            returnString = returnString + CHARACTERS[thisIndex];
        }
    ...
    public void flashSequence(String letters) {
        ...
        char[] characterOfThis = letters.toCharArray();
        for (int i = 0; i < characterOfThis.length; i++){
            drawFrame(Character.toString(characterOfThis[i]));
            StdDraw.pause(1000);
            drawFrame(" ");
            StdDraw.pause(500);
        }
    ...
    public void startGame() {
        ...
        while (!gameOver) {
            ...
            String input = solicitNCharsInput(thisString.length());
            if (!input.equals(thisString)){
                gameOver = true;
            }else{
                round++;
            }
